...
lib/
  ...
  conversation-topics.ts  최근/소진/미해결 대화 소재 상태 병합 + 프롬프트 힌트
...
6. "오늘 잠깐 만날래?", "이따 보자", "공원에서 보자", "산책 가자"처럼 실제 만남을 제안하고
   캐릭터가 받아들이면, 카톡 안에서 만남 장면을 길게 연기하지 않고 이벤트 카드로 넘긴 뒤
   시간대와 약속 성격에 맞는 카톡으로 복귀합니다. 데이트/카페/영화/밥/옷 고르기 맥락이면
   `둘은 데이트를 하고 돌아왔다`, 짧은 만남이면 `둘은 잠깐 만나고 돌아왔다`로 표시합니다.
...
- 최근 30개 메시지는 전체를 다시 보내지 않고, 유저가 최근 꺼낸 말/캐릭터 반응/최근 이벤트를 3줄 안팎의
  요약 힌트로 압축해 함께 전달합니다.
- 최근 대화 소재는 세션의 작은 JSONB 상태로 유지합니다. 충분히 다룬 소재는 AI가 먼저 다시 꺼내지 않고,
  미해결 약속/고민만 자연스러운 시점에 이어갈 수 있도록 기존 Claude 구조화 응답에 함께 포함합니다.
...
- 만남 이벤트는 실제 위치/장소 상태를 추적하지 않고, "약속 → 짧은 이벤트 카드 → 시간대별 카톡 복귀"로 처리
