...
function pickReply(personaType: PersonaType, kind: "laughter" | "ack", seed: string): string {
  const replies = LOCAL_REPLIES[personaType][kind] ?? LOCAL_REPLIES.default[kind];
  return replies[hashText(seed) % replies.length];
}
...
export function buildLocalShortReactionReply(
  ...
  if (/^[ㅋㅎ]+$/.test(compact)) {
    return {
      message: pickReply(personaType, "laughter", `${seed}:${message}:laughter`),
      emotion: "neutral",
      intensity: 0.2,
    };
  }
  ...
  if (/^(응+|웅+|ㅇㅇ+|어+|엉+|오키|오케이|ㅇㅋ|알겠어|알겟어|아하|아아)$/.test(compact)) {
    return {
      message: pickReply(personaType, "ack", `${seed}:${message}:ack`),
      emotion: "neutral",
      intensity: 0.15,
    };
  }
